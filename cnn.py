...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        ...
        if isinstance(self.model, keras.models.Sequential ):
            ...
            outputs = keras.layers.Dense(num_nodes, activation=activation, name=name)(inputs)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        self.model.add(keras.layers.Conv2D(num_of_filters, kernel_size,padding = padding, strides = strides, trainable = trainable, name = name, activation=activation))
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        ...
        return self.model.add(keras.layers.MaxPooling2D(pool_size=pool_size, strides = strides, name = name, padding = padding))
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if (layer_numbers != []):
            if (len(layer_numbers) == 0):
                self.model.layers[layer_numbers - 1].trainable = trainable_flag
            elif (len(layer_numbers) >= 1):
                for each_layer in layer_numbers:
                    self.model.layers[each_layer-1].trainable = trainable_flag
        elif (layer_names != "" ):
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        if (layer_number != None):
            ...
            try:
                ...
                return None
        return None if (weights is None) or (weights == []) else weights[0]
    ...
    def get_biases(self,layer_number=None,layer_name=""):
        ...
        if (layer_number != None):
            ...
            try:
                ...
                return None
        return None if (weights is None) or (weights == []) else weights[1]
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        """
        if layer_number!=None:
            self.model.layers[layer_number-1].set_weights(np.array([weights,self.get_biases(layer_number = layer_number)]))
        else:
            self.model.get_layer(layer_name).set_weights(np.array([weights,self.get_biases(layer_name = layer_name)]))
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        """
        if layer_number!=None:
             self.model.layers[layer_number-1].set_weights(np.array([self.get_weights_without_biases(layer_number = layer_number), biases]))
        else:
             self.model.get_layer(layer_name).set_weights(np.array([self.get_weights_without_biases(layer_name = layer_name), biases]))
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        if model_name == "VGG19" or model_file_name=="vgg19.hdf5":
            try:
                self.model =  keras.models.load_model("vgg19.hdf5",compile = False)

            except:
                self.model = tf.keras.applications.VGG19()
                self.model.save("vgg19.hdf5")

        elif model_name == "VGG16" or model_file_name=="vgg16.hdf5":
            try:
                self.model =  keras.models.load_model("vgg16.hdf5",compile = False)

            except:
                self.model = tf.keras.applications.VGG16()
                self.model.save("vgg16.hdf5")
        else:
            self.model = keras.models.load_model(model_file_name,compile = False)
        return self.model
...
if __name__ == "__main__":
    ...
    for layer in my_cnn.model.layers:
